...
function OnIcon($db,$fac_id){
	$fac_name = getFacName($db, $fac_id);
	switch ($fac_id){
			case 5://寒梅館
				echo '<form method="POST" action="map-i.php">
						<DIV style="position:absolute; top : 35px ; left : 610px ; ">
	   					<button type="button" onclick="clickIcon(5)" data-toggle="modal" data-target="#myModal" name="TC1"><img src="doushisha_icon.png" width="30px" height="30px" border="0"></button>
	   					</DIV>
						</form>';
				break;
			case 6://チャペル
				echo '<form method="POST" action="map-i.php">
						<DIV style="position:absolute; top : 350px ; left : 290px ; ">
	   					<button type="button" onclick="clickIcon(6)" data-toggle="modal" data-target="#myModal" name="TC2"><img src="doushisha_icon.png" width="30px" height="30px" border="0"></button>
	   					</DIV>
						</form>';
				break;
			case 7://未定
				echo '<form method="POST" action="map-i.php">
						<DIV style="position:absolute; top : 225px ; left : 415px ; ">
	   					<button type="button" onclick="clickIcon(7)" data-toggle="modal" data-target="#myModal" name="TC3"><img src="doushisha_icon.png" width="30px" height="30px" border="0"></button>
	   					</DIV>
						</form>';
				break;
			case 8://未定
...
foreach ($facility as $key => $value) {
	...
	$all_event_array[$value["id"]] = $fac_info;
  ...
  <script src="http://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>
	<script type="text/javascript">
		function clickIcon(fac_id){
			...
			<?php $all_event_json =  json_encode($all_event_array); ?>
			var all_event = JSON.parse('<?php echo $all_event_json ?>');
			
			place.innerHTML = all_event[fac_id]['fac_name'];
			name.innerHTML = '<form name="place" method="POST" action="./event.php"><input type="hidden" name="place" value="' 
...
<!-- 地図 //-->
<IMG src="imadegawa.png" width="90%" height="auto" border="0"> 
	<!--同志社アイコン-->
	<?php
